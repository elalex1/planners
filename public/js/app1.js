...
$("#formRequisitionEdit").validate({
    ...
    submitHandler: function (form) {
        ...
        newurl=quitarDiagRutaR(newurl,2)+'/update';
        $.ajax({
            type: "POST",
            //url: "/erp-web/requisicion/update",
            url: newurl,
            //dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.search("Folio") > 0) {
                ...
                newurl=quitarDiagRutaR(newurl,2);
                // setTimeout(document.location.href = "/erp-web/requisicion", 3000);
                setTimeout(document.location.href = newurl, 3000);
            }
    ...
    submitHandler: function (form) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (isNaN(response.id_requisicion)) {
                    M.toast({
                        html: response.id_requisicion
                    });

                } else {
                    M.toast({
                        html: 'Requisición guardada'
                    });
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.data == "exitoso") {
                ...
                let newurl=location.href;
                newurl=quitarDiagRutaR(newurl,2);
                // document.location.href = "/erp-web/requisicion";
                dicument.location.href = newurl;
            } else {
                M.toast({
                    html: response
                });
...
$(function () {
    ...
    $(".li-ppal-menu li a").each(function () {
        if ($(this).attr("href") == pgurl || $(this).attr("href") == '') {
            ...
            $(this)[0].parentElement.classList.add("teal")
...
function deleteRecord(item) {
    ...
    post['requisicion_id'] = requisicion_id;
    let newurl=location.href;
    newurl=quitarDiagRutaR(newurl,2)+'/deletearticle';

    $.ajax({
        type: "POST",
        // url: "/erp-web/requisicion/deletearticle",
        url: newurl,
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
...
function quitarDiagRutaR(text,n){
    let j=0;
    j=n;
    for (var i=0;i<j;i++){
        var n=text.lastIndexOf('/');
        var tx1= text.substring(n+1);
        text=text.replace('/'+tx1,'');
    }
    return text;
}
